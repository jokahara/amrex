...
void
AmrLevel::set_preferred_boundary_values (CellFabArray& S,
                                         int       state_index,
                                         int       scomp,
                                         int       dcomp,
                                         int       ncomp,
    ...
    : geom(level_geom), grids(ba), dmap(dm)
{
    ...
    {
        CellFabArray::RegionTag statedata_tag("StateData_Level_" + std::to_string(lev));
        CellFabArray::RegionTag statedata_index_tag("StateData_" + std::to_string(i) + "_Level_" + std::to_string(lev));
        CellFabArray::RegionTag statedata_index_new_tag("StateData_" + std::to_string(i) + "_New_Level_" + std::to_string(lev));
        CellFabArray::RegionTag level_tag("AmrLevel_Level_" + std::to_string(lev));
...
void
AmrLevel::setPhysBoundaryValues (CellFab&   dest,
                                 int        state_indx,
                                 Real       time,
                                 int        dest_comp,
                                 int        src_comp,
        ...
        else
        {
                                ...
                                boxGrow, boxType, desc.interp(SComp)))
            {
                ...
                fph = new FillPatchIteratorHelper(m_amrlevel,
                        m_leveldata, boxGrow, time, idx, SComp, NComp, desc.interp(SComp));
                               ...
                               int      idx)
{
    ...
    for (int l = 0; l < m_amrlevel.level; l++)
    {
        ...
        if (TheLevel.geom.isAnyPeriodic())
        {
            ...
            for (int i = 0; i < NC; i++)
            {
                ...
                for (const auto& iv : pshifts)
                {
                    ...
                    for (int j = 0; j < NC; j++)
                    {
                        const CellFab& srcfab = *CrseFabs[j];
        ...
        for (int ifine = 0; ifine < NF; ++ifine)
        {
            ...
            CellFab    crsefab(m_map->CoarseBox(finefab.box(),fine_ratio),m_ncomp);
            //
...
std::unique_ptr<CellFabArray>
AmrLevel::derive (const std::string& name, Real time, int ngrow)
{
    BL_ASSERT(ngrow >= 0);

    std::unique_ptr<CellFabArray> mf;

    int index, scomp, ncomp;

    if (isStateVariable(name, index, scomp))
    {
        mf.reset(new CellFabArray(state[index].boxArray(), dmap, 1, ngrow, MFInfo(), *m_factory));
    ...
    else if (const DeriveRec* rec = derive_lst.get(name))
    {
        ...
        CellFabArray srcMF(srcBA, dmap, rec->numState(), ngrow_src, MFInfo(), *m_factory);
        ...
        mf.reset(new CellFabArray(dstBA, dmap, dncomp, ngrow, MFInfo(), *m_factory));
...
#pragma omp parallel if (Gpu::notInLaunchRegion())
            ...
            for (MFIter mfi(*mf,TilingIfNotGPU()); mfi.isValid(); ++mfi)
            {
                ...
                CellFab& derfab = (*mf)[mfi];
                CellFab const& datafab = srcMF[mfi];
...
#pragma omp parallel
        ...
        for (MFIter mfi(*mf,true); mfi.isValid(); ++mfi)
        {
            ...
            for (MFIter mfi(srcMF); mfi.isValid(); ++mfi)
            {
                ...
                Cell*       ddat    = (*mf)[mfi].dataPtr();
                const int*  dlo     = (*mf)[mfi].loVect();
                const int*  dhi     = (*mf)[mfi].hiVect();
                int         n_der   = rec->numDerive();
                Cell*       cdat    = srcMF[mfi].dataPtr();
                const int*  clo     = srcMF[mfi].loVect();
                const int*  chi     = srcMF[mfi].hiVect();
                int         n_state = rec->numState();
                const int*  dom_lo  = state[index].getDomain().loVect();
    ...
    else
    {
        ...
        //
        std::string msg("AmrLevel::derive(CellFabArray*): unknown variable: ");
        msg += name;
...
void
AmrLevel::derive (const std::string& name, Real time, CellFabArray& mf, int dcomp)
{
...
#pragma omp parallel
            ...
            {
                int         idx     = mfi.index();
                Cell*       ddat    = mf[mfi].dataPtr(dcomp);
                const int*  dlo     = mf[mfi].loVect();
                const int*  dhi     = mf[mfi].hiVect();
                int         n_der   = rec->numDerive();
                Cell*       cdat    = srcMF[mfi].dataPtr();
                     ...
                     int        dcomp)
{
    ...
    CellFabArray::Copy(leveldata, mf_fillpatched, 0, dcomp, ncomp, boxGrow);
}
                        ...
                        int        dcomp)
{
    ...
    FillPatchIterator fpi(amrlevel, leveldata, boxGrow, time, index, scomp, ncomp);
    const CellFabArray& mf_fillpatched = fpi.get_mf();
    CellFabArray::Add(leveldata, mf_fillpatched, 0, dcomp, ncomp, boxGrow);
}
...
{
    // Build directory to hold the CellFabArrays in the StateData at this level.
