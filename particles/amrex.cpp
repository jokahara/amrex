...
void update_cell_lists(CellFabArray &grid, Geometry &geom) {
	...
	for (MFIter mfi(grid); mfi.isValid(); ++mfi) {
		...
		LoopOnCpu(box, [&] (int i, int j, int k)
		{
			...
			//std::cout << neighbor_cell << " to " << cell << ": " << neighbor_data->particles.size() << " particles: \n";
			while (n < neighbor_data->particles.size()) {				
				const IntVect current_cell = 
					geom.CellIndex( neighbor_data->particles[n].data() );
				
				if (current_cell == cell) {
					std::cout << "move " << neighbor_data->particles[n][0] << " from " << neighbor_cell << " to " << current_cell << "\n";
		...
		LoopOnCpu(box, [&] (int i, int j, int k)
		{
			...
			while (n < previous_data->particles.size()) {
				...
				if (current_cell == previous_cell) {
					...
					std::cout << "remove: " << previous_data->particles[n][0] << " from " << previous_cell << "\n";
...
void save(const int rank, CellFabArray &grid, unsigned int step)
{
	...
	for (MFIter mfi(grid); mfi.isValid(); ++mfi) {
		...
		{
			total_particles += a(i,j,k).number_of_particles;
		});
	}
...
int main_main()
{
    ...
    Geometry geom;
    {
        ...
        IntVect dom_hi(AMREX_D_DECL(2,0,0));
	...
	for (MFIter mfi(grid); mfi.isValid(); ++mfi)
	{
		...
		std::cerr << "Rank" << rank << " initialized box: " << box << "\n";
	}
	...
	*/
	...
	for (unsigned int step = 0; step < max_steps; step++) {
		...
		std::cout << "transfer sizes\n";
		Cell::transfer_particles = false;
		grid.FillBoundary(geom.periodicity()); // cross=true to not fill corners
		
		/*for (MFIter mfi(grid); mfi.isValid(); ++mfi)
		{
			const auto box = mfi.validbox();
			
			const auto &a = grid[mfi].array();

			Loop(box, [&] (int i, int j, int k)
			{
				IntVect idx(i,j,k);
				
				if(!grid.is_local(idx - IntVect(1,0,0)))
					std::cout << rank << ": " << a(idx - IntVect(1,0,0)).number_of_particles << " at " << idx - IntVect(1,0,0) << "\n";

				std::cout << rank << ": " << a(idx).number_of_particles << " at " << idx << "\n";
		...
		for (MFIter mfi(grid); mfi.isValid(); ++mfi)
		{
			...
			IntVect lo(box.loVect());
			IntVect hi(box.hiVect());
			if (!grid.is_local(lo - IntVect(1,0,0))) {
				box.growLo(0);
			}

			if (!grid.is_local(hi + IntVect(1,0,0))) {
				box.growHi(0);
			}
			...
			{
				IntVect idx(i,j,k);
				if(!grid.is_local(idx)) {
					grid[mfi].dataPtr(idx)->resize();
				}
			});
		}

		// update particle data between neighboring cells
		std::cout << "transfer particles\n";
